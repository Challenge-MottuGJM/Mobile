...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setErrors({ general: t('auth.errors.general') });
    } finally {
      setBusy(false);
    }
  };
    ...
    <LinearGradient colors={colors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ flex: 1 }}>
      <View style={styles.wrapper}>
        <Text style={[styles.title, { color: isDark ? '#fff' : '#333' }]}>
          {isLogged ? t('auth.account') : t('auth.signIn')}
        </Text>
        ...
        {isLogged ? (
            ...
            <View style={[styles.card, { backgroundColor: isDark ? '#1f1f1fcc' : '#ffffffcc' }]}>
              <View style={[styles.avatar, { backgroundColor: isDark ? '#2a2a2a' : '#EFE9FF' }]}>
                <Text style={[styles.avatarText, { color: isDark ? '#D6CCFF' : '#5e17eb' }]}>
                  {(state.user?.name || state.user?.email || 'U').toUpperCase().charAt(0)}
                </Text>
              </View>

              <Text style={[styles.name, { color: isDark ? '#fff' : '#2D2A32' }]}>
                {state.user?.name ?? t('auth.account')}
              </Text>
              ...
              <TouchableOpacity style={styles.primaryBtn} onPress={signOut} disabled={busy}>
                <Text style={styles.btnText}>{busy ? t('auth.signingOut') : t('auth.signOut')}</Text>
              </TouchableOpacity>
            </View>
          </View>
        ) : (
          <>
            ...
            <TouchableOpacity style={styles.primaryBtn} onPress={handleLogin} disabled={busy}>
              <Text style={styles.btnText}>{busy ? t('auth.signingIn') : t('auth.signIn')}</Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={() => router.push('/signup')}>
              <Text style={[styles.link, { color: '#5e17eb' }]}>{t('auth.createAccount')}</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 20, marginBottom: 4, fontFamily: 'Inter_700Bold' },
  email: { fontSize: 14, marginBottom: 20, fontFamily: 'Inter_400Regular' },
  input: {
    backgroundColor: '#fff',
    borderWidth: 1,
    borderColor: '#ccc',
